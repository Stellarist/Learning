...
int main(int argc, char const* argv[])
{
	...
	Shader phong_shader(SHADER_PATH "/sphere.vert", SHADER_PATH "/phong.frag");
	Shader blinn_phong_shader(SHADER_PATH "/sphere.vert", SHADER_PATH "/blinn_phong.frag");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		std::array<Shader*, 2> shaders = {&phong_shader, &blinn_phong_shader};
		...
		for (int i = 0; const auto& shader : shaders) {
			shader->use();
			shader->setMat4("model", glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(0.4f * (i == 0 ? -1 : 1), 0.0f, 0.0f)), glm::vec3(0.1f)));
			shader->setMat4("view", camera.getView());
			shader->setMat4("projection", camera.getProjection());
			shader->setVec3("light_pos", glm::vec3(0.0f, 0.0f, 5.0f));
			shader->setVec3("view_pos", camera.getPosition());
			shader->setVec3("object_color", glm::vec3(0.53f, 0.81f, 0.92f));
